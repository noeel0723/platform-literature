    ...
    return (
        <>
            ...
            <SiteContainer as="main" className="py-7 lg:py-10">
                <header className="border-b border-ink-950/20 pb-4">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                            <p className="text-xs font-bold uppercase tracking-[0.18em] text-brand-coral">{list.is_private ? 'Private list' : 'Public list'} · {list.items.total} literature{list.is_ranked ? ' · Ranked' : ''}</p>
                            <h1 className="mt-1 font-serif text-3xl font-bold text-ink-950">{list.title}</h1>
                            <p className="mt-1 text-sm text-ink-950/55">By <Link href={list.owner.url} className="font-bold text-ink-950 hover:text-brand-coral">{list.owner.name}</Link></p>
                        </div>
                ...
                {list.items.data.length === 0 ? <div className="py-12 text-center text-sm text-ink-950/50">This list is empty. Add literature from its detail page.</div> : (
                    <div className="mt-5 grid grid-cols-3 gap-x-3 gap-y-5 sm:grid-cols-5 md:grid-cols-7 lg:grid-cols-9">
                        {list.items.data.map((item, index) => item.literature && (
                            <article key={item.id} className="relative min-w-0">
                                {list.is_ranked && <span className="absolute -left-1.5 -top-1.5 z-10 grid size-7 place-items-center bg-ink-950 font-serif text-sm font-bold text-brand-cream shadow">{item.position}</span>}
