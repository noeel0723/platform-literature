...
export default function Show({ literature, viewer, ratingSummary, reviews, discussions, discussionCount, relationshipGroups, authorDiscoveries, reportReasons, successMessage, routes }) {
    ...
    return (
        <>
            ...
            <section className="relative isolate overflow-hidden border-b border-ink-950/10 bg-brand-cream">
                {literature.cover_url ? (
                    <>
                        <div className="absolute inset-0 -z-20 overflow-hidden" aria-hidden="true"><img src={literature.cover_url} alt="" className="size-full scale-110 object-cover object-center opacity-40 blur-sm" /></div>
                        <div className="absolute inset-0 -z-10 bg-linear-to-t from-brand-cream via-brand-cream/75 to-brand-cream/20" aria-hidden="true" />
                        <div className="absolute inset-0 -z-10 bg-linear-to-r from-brand-cream/95 via-transparent to-brand-cream/80" aria-hidden="true" />
                    </>
                ) : <div className="catalog-grid absolute inset-0 -z-10" aria-hidden="true" />}
                <div className="relative mx-auto max-w-7xl px-5 pb-14 pt-8 sm:px-8 lg:px-10 lg:pb-20 lg:pt-10">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <Link href={routes.catalog} className="inline-flex items-center gap-2 text-sm font-semibold text-ink-950/75 transition hover:text-brand-coral"><span aria-hidden="true">←</span> Back to catalog</Link>
                        {routes.admin_edit_metadata && <Link href={routes.admin_edit_metadata} className="rounded-full border border-ink-950/20 bg-brand-cream/80 px-4 py-2 text-xs font-bold uppercase tracking-[0.1em] text-ink-950 backdrop-blur transition hover:border-brand-coral hover:text-brand-coral">Edit metadata</Link>}
                    </div>
                    <div className="mt-24 grid gap-8 md:grid-cols-[220px_1fr] lg:mt-40 lg:grid-cols-[250px_minmax(0,1fr)_300px] lg:items-end">
