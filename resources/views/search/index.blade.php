                ...
                @if (in_array($scope, ['all', 'literature'], true) && $literatures->isNotEmpty())
                    <section id="literature-results" class="scroll-mt-24" aria-labelledby="literature-results-heading">
                        ...
                        @foreach ($literatures as $literature)
                            ...
                            @php($displaySynopsis = in_array(Str::lower((string) $literature->displayLanguage()), ['', 'en', 'eng', 'english'], true) ? $literature->displaySynopsis() : null)
                            <article class="grid grid-cols-[72px_minmax(0,1fr)] gap-4 border-b border-ink-950/10 py-5 sm:grid-cols-[86px_minmax(0,1fr)] sm:gap-6" data-search-literature>
                                <a href="{{ route('literatures.show', $literature) }}" class="aspect-[2/3] overflow-hidden rounded-sm border border-ink-950/15 bg-brand-sky/25">
                                    @if ($literature->displayCoverUrl())
                                        <img src="{{ $literature->displayCoverUrl() }}" alt="Cover of {{ $displayTitle }}" class="size-full object-cover" loading="lazy">
                                    @else
                                ...
                                <div class="min-w-0 self-center">
                                    <h2 class="font-serif text-2xl font-bold text-ink-950 sm:text-3xl"><a href="{{ route('literatures.show', $literature) }}" class="hover:text-brand-coral">{{ $displayTitle }}</a> @if ($literature->displayPublicationYear())<span class="font-sans text-base font-normal text-ink-950/45">{{ $literature->displayPublicationYear() }}</span>@endif</h2>
